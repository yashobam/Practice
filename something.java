...
public class Sudoku {    
    ...
    private boolean check(int r,int c,int num){
        for(int x=0;x<=8;x++){
            if (this.grid[r][x]==num){
                return false;
            }
        }
        for(int x=0;x<=8;x++){
            if (this.grid[x][c]==num){
                return false;
            }
        }
        /*int ro = r - r % 3, co = c - c % 3; 
        for (int i = 0; i < 3; i++) 
            for (int j = 0; j < 3; j++) 
                if (this.grid[i + ro][j +  co] == num) 
                    return false;*/
        return !subgridHasVal[r/3][c/3][num];
    ...
    private boolean solveRB(int n) {
        ...
        for (int i = 0; i < 9; i++) { 
            for (int j = 0; j < 9; j++) { 
                if (this.grid[i][j]==0 && !this.valIsFixed[i][j]) { 
                    r = i; 
                    c = j; 
                    something = false; 
                    break; 
        ...
        for (int num = 1; num <= 9; num++) { 
            if (this.check(r, c, num)) { 
                this.placeVal(num,r,c);
                if (solveRB(9)) { 
                    return true; 
                } 
                else { 
                    this.removeVal(num,r,c);
                } 
            } 
        } 
        return false; 
