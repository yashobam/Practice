...
public class RecursiveGraphics extends Canvas  {
   ...
   /* private void drawHTree(int depth, Graphics g) {
      ...
      private void drawHTree(int x,int y, int size,int l,Graphics g){
        if(l == 0)
            return;
        if(l%2!=0) {
          System.out.println((x-size/2)+" "+ y+" "+ (x+size/2)+ " "+ y+" "+l);
            drawLine(x-size/2, y, x+size/2, y, g);
            drawHTree(x-size/2, y, (int)size*(2/3), l-1, g);     
            drawHTree(x+size/2, y, (int)size*(2/3), l-1, g); 
        }                   
        else {
            drawLine(x, y-size/2, x, y+size/2, g);
            drawHTree(x, y-size/2, (int)size*(2/3), l-1, g);
            drawHTree(x, y+size/2, (int)size*(2/3), l-1, g);
        }
      }
   ...
   private double[] thetaDelta = {Math.PI/5, -Math.PI/5};   // add this delta to angle each time -- in radians
    ...
    public void paint(Graphics g) {
         ...
         drawHTree(windowSize/2,windowSize/2,windowSize/2,5,g);       // Problem 6.0.1
	// drawTree(3,g);        // Problem 6.0.2

	// drawFractalLine(windowSize*0.1, windowSize*0.3, windowSize*0.8, windowSize*0.6, 0, g);     // Problem 6.1	                                                                        
	// drawFractalSquare(windowSize*0.5,windowSize* 0.5,windowSize* 0.2, 0, g);                // Problem 6.2
